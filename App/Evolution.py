...
def memory():
	...
	return int(process.memory_info().rss/10**6)
...
class Evolution():
	...
	def on_finished(self):
		...
		print('Evolution is finished. ({:<.2f} s)'.format(self.end - self.start))
	...
	def optimize(self):
		...
		print('{:<20} {:<20} {:<20} {:<20} {:<20} {:<20}'.format('Generation', 'Best island', 'Train fitness', 'Val. fitness', 'Elapsed time [s]', 'Used memory [MB]'))
		...
		for iteration in range(self.config.OPTIM_ITER):
			...
			print('{:<20d} {:<20} {:<20.5f} {:<20.5f} {:<20.2f} {:<20d}'.format(iteration + 1, self.history[-1].best_island, self.history[-1].best_train_fitness, self.history[-1].best_val_fitness, end - start, memory()))
